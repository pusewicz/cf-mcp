...
function ToolExplorer() {
  ...
  const handleSubmit = async (e) => {
    ...
    setResult({ loading: true, loadingText: 'Executing...', error: null, content: null });
    ...
    try {
      ...
      if (data.error) {
        ...
        setResult({ loading: false, error: data.result.isError ? text : null, content: data.result.isError ? null : text });
      } else {
...
function TopicsExplorer() {
  ...
  const loadTopic = async (topicName) => {
    if (!topicName) {
      setResult({ loading: false, error: null, content: null });
      return;
    }

    setResult({ loading: true, loadingText: 'Loading topic...', error: null, content: null });
    ...
    try {
      ...
      if (data.error) {
        ...
        const content = data.result.content[0];
        const text = content.text || JSON.stringify(content);
        setResult({ loading: false, error: null, content: text });
      } else {
        setResult({ loading: false, error: null, content: JSON.stringify(data, null, 2) });
      }
    } catch (err) {
