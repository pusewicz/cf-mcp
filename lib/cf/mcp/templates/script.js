...
function initTopicsExplorer() {
  ...
  TOPICS.forEach(topic => {
    ...
    option.textContent = topic.name.replace(/_/g, ' ');
...
async function loadTopic() {
  ...
  resultArea.innerHTML = '<span class="loading">Loading topic...</span>';
  ...
  try {
    const response = await fetch('/http', {
      ...
      body: JSON.stringify({
        ...
        id: Date.now(),
        method: 'tools/call',
        params: { name: 'cf_get_topic', arguments: { name: topicName } }
      })
    });
...
// Store field values across tool changes
...
function restoreFieldValues() {
  for (const [name, value] of Object.entries(savedFieldValues)) {
    const field = document.getElementById('field-' + name);
    if (field) {
      // For select elements, only set if the option exists
...
function renderToolForm() {
  ...
  for (const [propName, propDef] of Object.entries(properties)) {
    ...
    if (propDef.enum) {
      ...
      html += '</select>';
    } else if (propName === 'name' && toolName === 'cf_get_topic') {
      // Render topic name as a dropdown
      html += '<select id="field-' + propName + '" class="form-select" data-type="string"' + (isRequired ? ' required' : '') + '>';
      html += '<option value="">-- Select Topic --</option>';
      TOPICS.forEach(topic => {
        html += '<option value="' + topic.name + '">' + topic.name.replace(/_/g, ' ') + '</option>';
      });
      html += '</select>';
    } else {
